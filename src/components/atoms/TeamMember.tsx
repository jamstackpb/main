...
const MemberContainer: React.FC<{ memberRankId: string }> = ({ children, memberRankId }) => {
    if (memberRankId == '2') {
        ...
        return <div className="w-92 max-w-full bg-primaryBackgroundColor rounded-3xl px-1 m-1 shadow-lg hover:bg-transparent hover:shadow-2xl">{children}</div>
    } else return null;
};

const MemberImg: React.FC<{ src: string; alt: string }> = ({ src, alt }) => (
    <img
        className="bg-white h-84 max-h-84 w-92 mx-auto rounded-t-3xl mt-1"
        src={src}
        alt={alt}
    />
);
...
const MemberFigureCaption: React.FC = ({ children }) => (
    <figcaption className="font-medium text-center bg-tertiaryBackgroundColor max-w-92 h-32 mx-auto rounded-b-3xl">{children}</figcaption>
);
...
const TeamMember: React.FC<TeamMemberProps> = ({ memberName, memberRank, memberRankId, memberDescription, memberGIT, imgPath }) => {
    return (
